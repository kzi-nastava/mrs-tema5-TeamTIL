...
public class RideStatsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_ride_stats, container, false);
        bindViews(view);
        setupDatePickers();
        setupQuickFilters();
        // Load last 30 days by default, same as frontend
        applyLast30Days();
        return view;
    }
    ...
    private void showDatePicker(boolean isFrom) {
        ...
        new DatePickerDialog(requireContext(), (view, year, month, day) -> {
            ...
            if (isFrom) {
                ...
                apiDate = API_FORMAT.format(selected.getTime());
    ...
    private void setupQuickFilters() {
        btnLast7Days.setOnClickListener(v -> {
            applyQuickFilter(-7);
            highlightButton(btnLast7Days);
        });
        btnLast30Days.setOnClickListener(v -> {
            applyLast30Days();
            highlightButton(btnLast30Days);
        });
        btnLast3Months.setOnClickListener(v -> {
            applyQuickFilter(-90);
            highlightButton(btnLast3Months);
        });
    }
    ...
    private void loadStats() {
        ...
        rideApi.getUserStats(email, selectedFromDate, selectedToDate).enqueue(new Callback<RideStatsResponseDTO>() {
            @Override
    ...
    private void bindStats(RideStatsResponseDTO stats) {
        ...
        boolean isEmpty = stats.getTotalRides() == 0;
    ...
    private void setupChart(LineChart chart, List<RideStatsDayDTO> days, String dataType, int color) {
        ...
        for (int i = 0; i < days.size(); i++) {
            ...
            // Format label - show month/day only
            String date = day.getDate();
            if (date != null && date.length() >= 10) {
                labels.add(date.substring(5, 10).replace("-", "/"));
            } else {
                labels.add(date != null ? date : "");
            }
        }
        ...
        // Show ~6 labels across the chart to avoid overcrowding
